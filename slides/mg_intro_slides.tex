...
\begin{itemize}
    ...
    \onslide<3->{\item \textbf{Algebraic Case (Galerkin approach):} Given prolongation $P$ and setting $e \approx Pe_{c}$:}
    \end{itemize}
\onslide<3->{
    \begin{align*}
...
\begin{align*}
    \onslide<1->{\text{Pre-smoothing ($q$ steps):} & \quad e \leftarrow S^{q}e \\}
    \onslide<2->{\text{Compute residual:}          & \quad r = Ae \\}
    \onslide<3->{\text{Restrict residual:}         & \quad r_{c} = P^{\top}r \\}
    \onslide<4->{\text{Exact coarse solve:}        & \quad e_{c} = A_{c}^{-1}r_{c} = {(P^{\top}AP)}^{-1}r_{c} \\}
    \onslide<5->{\text{Interpolate \& Correct:}    & \quad e \leftarrow e - Pe_{c}}
\end{align*}
...
\[
    e \leftarrow \left(I - P{(P^{\top}AP)}^{-1}P^{\top}A\right)S^{q}e
\]
}
\onslide<7->{
The \textbf{Two-Grid Error Operator} is:
\[
    TG_{q} = \left(I - P{(P^{\top}AP)}^{-1}P^{\top}A\right)S^{q}
\]
}
\end{frame}
